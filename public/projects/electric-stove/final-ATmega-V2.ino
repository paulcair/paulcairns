...
const int CurrentPin = A1;    // pin where current sensor is conncected
const int VoltPin = A0;        // pin where voltage sensor is connected
...
const int LEDPIN = 7;        // LED pin for Adafruit
const int NUMPIXELS = 8;     // Number of pixels on the Adafruit
int Voltbits;                // Bit value of the voltage reading from the volt sensor for I2C
int Currbits;                // Bit value for the current reading from the current sensor for I2C
int Tempbits;                // Bit value for the temperature reading from the temp sensor for I2C
...
//Function to perform when an I2C request is made from ESP32
void requestEvent(){
...
void setup() {
  ...
  //SEND BIT VALUES FOR VOLTAGE CURRENT AND TEMPERATURE TO ESP32 VIA I2C when requested
...
//GET AND PRINT CURRENT
  ...
  //Map the current value to a value that can be sent via I2C
    Currbits = map(Current,-5,5,1,255);
...
//GET AND PRINT TEMPERATURE
  ...
  //Map the temperature to a value that can be sent via I2C
    Tempbits = map(tempC,0,125,1,255);
